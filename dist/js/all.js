...
document.querySelectorAll('[data-css]').forEach(function (item) {
...
window.addEventListener('keyup', function (e) {
	if (e.keyCode === 13 && e.ctrlKey === true) {
		var url = location.host.replace(/^(www|local|log|dev)\.|\.(com|co|es|edu|gov|info|org|net|uol\.com)(|\.(br|))$/gi, '');
...
var Icons = function Icons() {
    ...
    url.map(function (item, i) {
        fetch(item.url).then(function (res) {
            ...
            array.forEach(function (icon) {
                return iconArray.push(icon.replace(/\s/g, '').split(':before{content:'));
            });

            var grid = document.getElementById('font-grid-' + i);
            var _iteratorNormalCompletion = true;
            var _didIteratorError = false;
            var _iteratorError = undefined;

            try {
                for (var _iterator = iconArray[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    var icon = _step.value;

                    var name = icon[0].replace('.', '').replace(/:before\,\./g, ', '),
    ...
    document.querySelector('[data-search]').addEventListener('input', function (event) {
        ...
        document.querySelectorAll('.io-icon-item').forEach(function (item) {
...
var Maskfy = function () {
    ...
    _createClass(Maskfy, [{
        ...
        value: function input(_input) {
            ...
            _input.addEventListener('input', function (e) {
                var inputValue = _input.value,
                    ...
                    mask = _this.objectMask || _input.dataset.mask,
                    maskLength = mask.length,
                    maskSize = _this.objectSize || _input.dataset.maskSize,
                    value = inputValue.replace(/\D/g, ''),
                    valueFinal = value.split(''),
                    objectReverse = _this.objectReverse || _input.hasAttribute('data-mask-reverse');
                var i;
                ...
                _input.value = valueFinal.join('');
                if (deviceType() === 'mobile' || deviceType() === 'tablet') {
                    _this.responsive(_input);
                }
            });
        }
    }, {
        ...
        value: function format(input) {
            var inputTrigger = input.value.split(''),
            ...
            input.value = '';
            inputTrigger.map(function (item) {
                input.value += item;
                var eventTrigger = new MouseEvent('input');
                input.dispatchEvent(eventTrigger);
            });
        }
    }, {
        key: 'responsive',
        value: function responsive(input) {
            setTimeout(function () {
...
var Scrolly = function () {
	...
	_createClass(Scrolly, [{
		...
		value: function scroll() {
			...
			var JSONScrolly = JSON.parse(sessionStorage.getItem('scrolly')),
			    y = window.scrollY;
		...
		value: function after() {
			var click = document.querySelector('[data-scrolly-click]'),
...
var Slidriks = function () {
  function Slidriks(tag) {
    var _this = this;

    _classCallCheck(this, Slidriks);

    document.querySelectorAll(tag).forEach(function (item, i) {
      var json = { "id": "slidriks" + i, "views": 4, "width": item.offsetWidth, "navigate": true, "paginate": true, "childSize": item.children.length, "childWidth": item.offsetWidth / 4 };
  ...
  _createClass(Slidriks, [{
    key: "init",
    value: function init(item) {
      var contentHTML = item.innerHTML,
      ...
      item.querySelectorAll('.slidriks--content > *').forEach(function (li) {
    ...
    value: function navigate(json) {
      ...
      item.querySelectorAll('.slidriks--side').forEach(function (side) {
        side.addEventListener('click', function (event) {
          var contentX = parseFloat(item.querySelector('.slidriks--content').style.transform.replace(/translateX\(|px\)/g, ''));
          if (side.classList.contains('slidriks--side-prev')) {
            if (contentX >= 0) return;
            _this2.slide(json, contentX + json.childWidth);
          } else {
    ...
    key: "paginate",
    value: function paginate(json) {
      var _this3 = this;

      var pageLoop = Math.floor(json.childSize / json.views) + (json.childSize % json.views !== 0 ? 1 : 0),
          item = window[json.id],
      ...
      item.querySelectorAll('.slidriks--page').forEach(function (page) {
        page.addEventListener('click', function (event) {
          var index = json.views * parseInt(page.textContent),
          ...
          _this3.slide(json, -itemLeft);
        });
      });
    }
  }, {
    key: "slide",
    value: function slide(json, value) {
      var content = window[json.id].querySelector('.slidriks--content');
    ...
    key: "mousemove",
    value: function mousemove(json) {
      var item = window[json.id],
          content = item.querySelector('.slidriks--content');
      document.addEventListener('mousemove', function (event) {
        if (content.classList.contains('slidriks--mousemove')) {
          var current = parseFloat(content.dataset.slidriksX),
    ...
    key: "mouseup",
    value: function mouseup(json) {
      var _this4 = this;

      var item = window[json.id],
          content = item.querySelector('.slidriks--content');
      'mouseleave mouseup'.split(' ').map(function (event) {
        document.addEventListener(event, function () {
          if (content.classList.contains('slidriks--mousemove')) {
            var current = parseFloat(content.style.transform.replace(/translateX\(|px\)/g, '')),
...
var Validity = function () {
    ...
    _this.validityCustom = {
      ...
      cnpj: {
        fn: function fn(input) {
          ...
          var resultado, tamanho, numeros, digitos, soma, pos, i;
  ...
  _createClass(Validity, [{
    ...
    value: function init() {
      var _this = this;
      _this.element.forEach(function (tag) {
        tag.querySelectorAll('input, textarea, select').forEach(function (input) {
    ...
    key: 'attributes',
    value: function attributes(input) {
      var attributes = ['min', 'max'],
      ...
      attributes.map(function (item) {
        var attr = input[item];
        if (attr) {
          var datacustom = item[0].toUpperCase() + item.slice(1);
    ...
    key: 'validityAddClass',
    value: function validityAddClass(input) {
      input.classList.add('validity--input-failed');
    }
  }, {
    key: 'validityRemoveClass',
    value: function validityRemoveClass(input) {
      input.classList.remove('validity--input-failed');
    }
  }, {
    key: 'validity',
    value: function validity(tag) {
      var _this = this;
      tag.querySelectorAll('input, textarea, select').forEach(function (input) {
        var type = input.getAttribute('data-validity-type') || input.type,
        ...
        if (input.hasAttribute('data-validity-max')) {
          var max = parseFloat(input.getAttribute('data-validity-max'));
          if (input.type === 'file' && input.files.length > 0) {
            Object.keys(input.files).map(function (item) {
              if (max < input.files[item].size / 1024) {
                _this.validityAddClass(input);
                msgError.push('Arquivo deve ter at\xE9 ' + max + ' Kb');
              }
            });
          } else {
            if (max < parseFloat(value)) {
              _this.validityAddClass(input);
              msgError.push('Valor m\xE1ximo at\xE9 ' + max);
            }
          }
        }
        if (input.hasAttribute('data-validity-min')) {
          var min = parseFloat(input.getAttribute('data-validity-min'));
          if (input.type === 'file' && input.files.length > 0) {
            Object.keys(input.files).map(function (item) {
              if (min > input.files[item].size / 1024) {
                _this.validityAddClass(input);
                msgError.push('Arquivo deve ter no m\xEDnimo ' + min + ' Kb');
              }
            });
          } else {
            if (min > parseFloat(value)) {
              _this.validityAddClass(input);
              msgError.push('Valor m\xEDnimo de ' + min);
            }
          }
        }
        if (input.hasAttribute('data-validity-custom')) {
          try {
            var custom = _this.validityCustom[input.getAttribute('data-validity-custom')];
            custom.fn(input);
        ...
        if (input.hasAttribute('data-validity-accept') && input.files.length > 0) {
          var accept = input.getAttribute('data-validity-accept').replace(/,/g, '|'),
              regex = new RegExp('(' + accept + ')$');

          Object.keys(input.files).map(function () {
            if (!regex.test(value)) {
              _this.validityAddClass(input);
    ...
    key: 'setMensage',
    value: function setMensage(msgArray, input) {
    ...
    key: 'validityCheck',
    value: function validityCheck(form) {
      var _this = this;
      if (!form.querySelector('.validity--input-failed')) {
        _this.send(form);
      } else {
        document.querySelectorAll('.validity--notice').forEach(function (item) {
          item.remove();
        });
        form.querySelectorAll('.validity--input-failed').forEach(function (input, i) {
          ...
          alertbox.addEventListener('mouseover', function (event) {
    ...
    value: function send(form) {
      ...
      xhr.addEventListener('loadend', function (e) {
        ...
        var close = document.querySelector('.validity--close'),
            closeEvent = setTimeout(function () {
          var eventTrigger = new MouseEvent('click');
          close.dispatchEvent(eventTrigger);
        }, 3000);
        close.addEventListener('click', function (event) {
          status.classList.add('validity--status-remove');
          var remove = document.querySelector('.validity--status-remove'),
